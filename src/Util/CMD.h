...
public:
    ...
    OptionParser(const OptionCompleted& cb = nullptr, bool enable_empty_args = true) {
        ...
        _helper = Option('h', "help", Option::ArgNone, nullptr, false, "print option message",
                        [this](const std::shared_ptr<std::ostream>& stream, const std::string& arg)->bool {
            
             static const char* argsType[] = { "ArgNone", "ArgRequired", "ArgOptional" };
             static const char* mustExist[] = { "optional", "must" };
...
public:
    ...
    void operator()(const std::string& line, const std::shared_ptr<std::ostream> &stream = nullptr) {
        ...
        if (it == _cmd_map.end()) {
            std::stringstream ss;
            ss << "  unrecognized command\"" << cmd << "\",print \"help\" for help.";
...
public:
    ...
    const char* description() const override {
        return "print the help information";
    }
};
...
public:
    ...
    const char* description() const {
        return "clear the screen output";
    }
